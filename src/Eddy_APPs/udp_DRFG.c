...
int i;
	...
	for ( i=7; i<len ; i++ ) {
		...
		//(j < 0 ? -j : j)
		WORK2[(i-7)*2 ]   = ( (WORK[i]>>4) < 10 ? (WORK[i]>>4) + 0x30 : ((WORK[i]>>4)-10) + 0x41 );
		WORK2[(i-7)*2+1 ] = ( (WORK[i]&0xf) < 10 ? (WORK[i]&0xf) + 0x30 : ((WORK[i]&0xf)-10) + 0x41 );
	}
	WORK2[ (i-7)*2 ] = 0;  //0x0d;   // WORK2[ i*2+1 ] = 0;


	if (protocol == SB_UDP_SERVER_MODE)	// server
		SB_SendUdpServer (SYS.lfd, WORK, len);
	else   //// <-----SB_DISABLE_MODE---------------{udp_DRFG}!!
		SB_SendUdpClient (SYS.lfd, WORK2, (len-7)*2, SYS.server_ip, SYS.server_port);
	...
	//---吝汗贸府---//  
	if ( (RX_last[0] != 0) && ((SB_GetTick() - RX_lastTick) < 10000) )
	{
		if ( !memcmp( RX_last, &WORK[7], 9 ) )
		{
			if (RX_LastN > 2)  return 0; //==>Cancle!
		}
		else { 	RX_LastN=0; 	}
	}
	else {		RX_LastN=0; 	}
	...
	//=======================================>> To-RF-Modem!!!
	//
	memcpy (RX_last, WORK2, len-7);  //---吝汗贸府---//
