...
#define MAX_BUFFER_SIZE 		13000  ///3000
#define MAX_SOCKET_READ_SIZE 	10000  ///512
...
char WORKt [100];
	...
	****/
	...
	/****
	if (  strncmp( CFG_SYS.device_name, "DPS2590", 7) == 0 ) 
	{
			// DPS---RUN!
			SB_SendUdpClient (SYS.lfd, DPS_SCAN, 16, SYS.server_ip, SYS.server_port); 
	}
	****/
		...
		//@ loop-Timer @//
			...
			if ( lpTimer < lpTimerC )
			{
				...
				if (  strncmp( CFG_SYS.device_name, "DPS2590", 7) == 0 ) 
				{
					...
					if ( DPS_Lcnt > 3 )  ///---(4)==2sec---
					{
						DPS_Lcnt = 0;
						
						// SB_SendUdpClient (SYS.lfd, DPS_GSCN, 16, SYS.server_ip, SYS.server_port);
					}
				}
...
//---------------------------
	...
	len = SB_ReadUdp (SYS.lfd, WORK, MAX_SOCKET_READ_SIZE);
	...
	/****
	< PS_Programmers_Manual_V03-03-19.docx>
	-------------Response from the sensor:
	0000 < GSCN >
	0004 < length >
	0008 < number of parameters (variable) >
	0012 < 1st parameter: scan number >
	0016 < 2nd parameter: time stamp >
	0020 < 3rd parameter: Scan start direction >
	0024 < 4th parameter: Scan angle >
	0028 < 5th Parameter: number of echoes per point >
	0032 < 6th parameter: Incremental count >
	0036 < 7th parameter: system temperature >
	0040 < 8th parameter: system status >
	0044 < 9th parameter: Data content>
		{{ < 10th parameter: scan line number > }}
	0048 < Number of points (variable) >
	0052 < Array with "n" measurements [n = number_of_points * number_of_echos_per_point]>
	8052 < CRC >
	-------------
	****/
	...
	if (  strncmp( CFG_SYS.device_name, "DPS2590", 7) == 0 ) 
	{
		...
		if(len > 256)
		{
			...
			while (len > 256)
			{
				...
				{
					sio_len = ioctl (SYS.sfd, TIOTGTXCNT);
					if (sio_len >= 256) break;
					SB_msleep (1);
				}
				//
				SB_SendSerial (SYS.sfd, cp1, 256);
				len -= 256;
				cp1 += 256;
				//
				////SB_msleep (50);  //<==[115200bps]
				//
				if (len <= 256) 
				{
					SB_msleep (20);
					SB_SendSerial (SYS.sfd, cp1, len);  //..last!
					SB_msleep (30);	 // delay 20 msec 
					break;
				}
